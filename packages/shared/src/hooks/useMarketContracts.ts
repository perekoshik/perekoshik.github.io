...
export function useMarketContracts() {
  ...
  const makeShop = useCallback(
    async (name: string) => {
      ...
      if (!token) {
        throw new Error('Авторизуйтесь в консоли продавца, чтобы сохранить магазин.');
      }
    ...
    [client, sender, wallet, connected, telegramUser, refreshShopInfo, token, wrongNetwork],
  );
  ...
  const makeItem = useCallback(
    async (params: {
      ...
      if (!token) {
        throw new Error('Авторизуйтесь в консоли продавца, чтобы публиковать товары.');
      }
      if (wrongNetwork) {
        throw new Error(`Переключите кошелёк в ${TARGET_NETWORK_LABEL}`);
      }
    ...
    [client, sender, shopAddress, shopItemsCount, refreshShopInfo, refreshItems, token, wrongNetwork],
  );
...
async function persistShopRecord(record: { address: string; owner: string; shopName: string }, token: string) {
  try {
    await Api.saveShop(token, {
...
async function persistItemRecord(
  record: {
    id: string;
    shopAddress: string;
    title: string;
